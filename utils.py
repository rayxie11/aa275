...
def match_bi_img(bi_loc, img_loc, e=0.5):
    bi_file = os.listdir(bi_loc)
    img_file = os.listdir(img_loc)
    
    single = "C:/Users/ray_s/Desktop/Navigation for Autonomous Systems/Project/aa275_project/results/1508987772102887.png"
    img = cv2.imread(single)
    img_grey = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    #edges = cv2.Canny(img,200,270)
    #print(edges)
    #cv2.imshow('edges',edges)
    #lines = cv2.HoughLinesP(edges,1,np.pi/180,100,100,10)
    lines = cv2.HoughLines(img_grey,1,np.pi/180,200)
    #lines = cv2.HoughLinesP(edges,1,np.pi/180,0,65,10)
    print(lines.shape)
    print(lines)
